...
int TclgrCmd (ClientData clientData, Tcl_Interp *interp, int argc, 
	      const char *argv[])
{
    if (system("which xmgr > /dev/null 2>&1")) {
    	printf("xmgr is not installed in any binary path including the solar bin directory\n");
    	printf("xmgr can be installed by building the code located at: https://github.com/mlund/xmgr-resurrection \n");
    	printf("Once built place xmgr in the solar bin directory or a directory found in environment variable PATH\n");
    	return TCL_ERROR;
    }
