...
//	printf ("TESTING MODELTYPE\n");	

	if (Strcmp("autophi2",Option::get_string("ModelType")) ||
	(Trait::Maximization_Type() == discrete && 
	 Option::get_int ("EnableDiscrete")))
    {
	if (Premax) fprintf (stderr, "    **  Checking if phi2 matrix loaded\n");
	...
	if (message)
	{
	    fprintf (stderr, "%s\n", message);
	    RESULT_LIT (
	"Error loading phi2.gz required unless option modeltype autophi2");
	    return TCL_ERROR;
	}
    }
    int special_type = !(!Strcmp("Default",Option::get_string("ModelType"))
		      || !Strcmp("autophi2",Option::get_string("ModelType")));
    if (special_type ||
	(Trait::Maximization_Type() == discrete && 
	 Option::get_int ("EnableDiscrete")))
    {
