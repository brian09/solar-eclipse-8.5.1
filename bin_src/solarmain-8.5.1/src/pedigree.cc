		  ...
		  int argc, char *argv[])
{
    ...
    if (argc == 2 && !StringCmp ("hassex", argv[1], case_ins)) {
	if (Pedigree::HasSex())
	{
	    RESULT_LIT ("1");
	} else {
...
// pedindex now tested for sex variable
// this requires opening now (as later) in tablefile
// because there is potential of different file formats
//  (user could, in theory, force csv file as pedindex)

    Pedigree::SexVar(0); // toggle when we actually find "sex"
    const char *errmsg = 0;
    TableFile* pedindex = TableFile::open("pedindex.out", &errmsg);
    if (errmsg) {
	return false; // no pedindex or invalid
    }
    if (pedindex->test_name("SEX",&errmsg)) {
	if (!errmsg) {
	    Pedigree::SexVar(1);  // Found!
	}
    }
    delete pedindex;
    pedindex = 0;
    ...
    const char* stats_errmsg;
    if (!currentPed->get_stats(&stats_errmsg)) {
        delete currentPed;
        throw Safe_Error_Return(stats_errmsg);
    }
}
