...
// argument after all the hyphenated arguments.
	...
	if (argc==3 || (argc>3 && (!Strcmp ("-noheader", argv[2]) ||
				   ...
				   !Strcmp ("-rows", argv[2]))))
	{
	    int status = MathMatrixCmd (clientData, interp, argc, argv);
	    if (status!=TCL_OK) {
		if (argc==3) {
		    RESULT_LIT (
"File is not a valid MathMatrix.  If you intended to load PHI2/IBD matrix\n\
  must give command like this: load matrix <filename> <matrixname>");
		    return TCL_ERROR;
		}
	    } else {
		printf ("Loaded as mathmatrix (no phi or ibd name given)\n");
	    }
	    return status;
	}
    }
